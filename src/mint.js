...
function buildManualMintTx(config, wallet) {
  const value = ethers.parseEther(String(config.mintPriceEth)) * BigInt(config.quantity);
  ...
  const data = new ethers.Interface(['function mint(uint256 quantity) payable']).encodeFunctionData('mint', [
    BigInt(config.quantity),
  ]);
