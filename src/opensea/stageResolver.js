...
async function fetchOpenSeaStages(config, logger) {
  ...
  try {
    const response = await fetch('https://gql.opensea.io/graphql', {
      method: 'POST',
      headers,
      body: JSON.stringify({ operationName: 'DropBySlug', query: STAGES_QUERY, variables: { slug: config.openSeaCollectionSlug } }),
    });
...
function activeStage(stages, now = Date.now()) {
  for (const stage of stages) {
    ...
    if (now >= start && now <= end) return { ...stage, selectedMode: classifyStage(stage) };
  }
  const next = stages
    .map((stage) => ({ ...stage, startMs: stage.startTime ? Date.parse(stage.startTime) : Infinity }))
...
async function resolveStage(config, logger) {
  ...
  if (selected) logger.info('OpenSea stage resolved', { name: selected.name, type: selected.stageType, selectedMode: selected.selectedMode });
